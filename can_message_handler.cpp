...
public:
  static constexpr uint16_t ARBID = 1F0;
  uint16_t id() const override { return ARBID; }
  void parse(const tCAN &msg, CarStatus &status) override {
    w1_raw = msg.data[0] + ((msg.data[1] & 0x0F) << 8);
    w1_kph = w1_raw / 16.0;
    w1_mph = w1_raw / 10.0;

    w2_raw = msg.data[2] + ((msg.data[3] & 0x0F) << 8);
    w2_kph = w2_raw / 16.0;
    w2_mph = w2_raw / 10.0;

    w3_raw = msg.data[4] + ((msg.data[5] & 0x0F) << 8);
    w3_kph = w3_raw / 16.0;
    w3_mph = w3_raw / 10.0;

    w4_raw = msg.data[6] + ((msg.data[7] & 0x0F) << 8);
    w4_kph = w4_raw / 16.0;
    w4_mph = w4_raw / 10.0;
...
constexpr size_t NUM_HANDLERS = 6;
...
void dispatch_can(const tCAN &msg, CarStatus &status) {
  for (auto *h : handlers) {
    if (h->id() == msg.id) {
      h->parse(msg, status);
      return;
